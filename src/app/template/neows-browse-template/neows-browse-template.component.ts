  ...
  reloadNeoWsBrowse(): void {
    ...
    this.http.get(this.infrastructureApi.ResponceURLDict[this.infrastructureApi.baseServiceName][this.infrastructureApi.baseService]).subscribe(
      (body) => {
        ...
        // tslint:disable-next-line: max-line-length
        this.infrastructureCommonTable.makeTableDef(this.serviceResponseBodyList, this.infrastructureApi.baseService, `Total Element: ${this.totalNoOfElements}`, 2, 'near_earth_objects');
      },
      (error: any) => {
        console.log(error);
      },
